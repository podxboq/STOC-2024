...
\email{jescanez@ull.edu.es}
\orcid{0000-0003-4215-0501}
...
\authornotemark[1]
\email{francisco.cc@qubitalent.com}
\affiliation{%
	\institution{Qubitalent S.L.U.}
	\city{Murcia}
	\country{Spain}}
\orcid{0000-0002-7669-8930}
%\end{comment}
%%
...
%Añadir un estado del arte comentando las diferencias con trabajos con similar objetivo, como \cite{gerdt}, \cite{Hutsell}, \cite{doi:10.1098/rsta.2019.0161}, \cite{Staton}, \cite{Wang19}, \cite{Wang21} and \cite{Ying}.
...
%%%esta notación queda un poco raro introducirla en este momento pero supongo que hace falta
...
%A ket is of the form $\ket{a}$ and denotes a vector $a$ in a vector space $V$, which physically represents a state of some quantum system.
...
%However, when an arbitrary number of indexes is used,  the following notation is preferable:
\begin{notation}
...
\end{notation}
%A gate is defined throughout Hilbert space, but sometimes its influence affects only some qubits, then we want to write the gate out of the ket, we indicate in which position the gate has been applied with a subscript.
...
% \section{Examples of circuits like algebraic circuits}
% Now we go to show some examples of circuits write like algebraic circuit:
% \begin{enumerate}
% 	\item Bell state: $\gate{H}_1\stategate{X}{1}{2}$.
% 	\item Superdense coding: $\gate{H}_1 \stategate{X}{1}{2}\gate{X}_1\stategate{X}{1}{2}\gate{H}_1$.
% 	\item Teleportation: $\gate{H}_2\stategate{X}{2}{3}\stategate{X}{1}{2}\gate{H}_1$.
% 	\item Bit flip error correction: $\stategate{X}{1}{2}\stategate{X}{1}{3}\stategate{E}{}{1,2,3}\stategate{X}{1}{2}\stategate{X}{1}{3}\stategate{X}{2,3}{1}$.
% \end{enumerate}
...
% Se ha realizado el desarrollo de un módulo de Python independiente para la simulación de circuitos cuánticos, adaptado a la notación previamente descrita. A partir del código fuente en \LaTeX{} para la descripción de estos circuitos, siguiendo las intrucciones de la propuesta, se obtiene el cálculo automático del mismo, logrando el vector de estado resultante. Además, cuenta con diferentes funcionalidades para la representación del circuito y su vector de estado asociado. La gramática libre del contexto que genera el lenguaje propuesto es la siguiente:
...
% El símbolo no terminal TAG define el nombre de la puerta con letras mayúsculas (al derivarse en secuencias de LETTER). Cada puerta viene siempre acompañada de una cantidad de argumentos (ARGS); que son los objetivos y controles de la misma. El orden de los argumentos no es importante, y el argumento de control es opcional. En ambos casos, si el NUMBER es argumento es único, puede prescindir de las llaves que lo encierran. En otro caso, podrá tratarse de un rango de índices, que es continuo o discontinuo gracias a la regla de producción de RANGE, y el uso del SEPARATOR correspondiente.
...
% Una vez definida esta gramática, se ha construido un Parser capaz de reconocer y comprender instancias generadas mediante estas estructuras sintácticas, transformándolas en objetos de programación funcionales. Además de ello, se ha implementado un Writer capaz de generar el fichero con código \LaTeX{} que desarrolla la expresión introducida paso a paso, calculando el vector final de estado. Inicialmente, el programa principal permite establecer un número de qubits en el sistema, y la codificación del circuito deseado utilizando este lenguaje. Esto permite obtener el vector de estado generado, así como la visualización del mismo utilizando Circle Notation. También realiza la visualización gráfica del circuito introducido. Todas estas funcionalidades han sido desarrolladas de manera original, sin utilizar frameworks o librerías externas de simulación de algoritmos cuánticos como Qiskit, Pennylane, Q\#, Amazon Braket, Cirq, etc. Se ha implementado de manera indepentiente un Parser, un Writer, un motor de ejecución, visualizadores de circuitos y vectores de estado.
...
\begin{figure}[!htb]
\centerline{\includegraphics[width=0.28\textwidth]{../bell_state.eps}}
\caption{Circle Notation of Bell State.}
\label{bell_state}
\end{figure}
\item Graphical representation (Fig.~\ref{bell_state_qc}).
\begin{figure}[!htb]
\centerline{\includegraphics[width=0.18\textwidth]{../bell_state_qc.png}}
...
\begin{figure}[!htb]
\centerline{\includegraphics[width=0.3\textwidth]{../superdense_coding.eps}}
...
\begin{figure}[!htb]
\centerline{\includegraphics[width=0.3\textwidth]{../superdense_coding_qc.png}}
...
\begin{figure}[!htb]
\centerline{\includegraphics[width=0.5\textwidth]{../teleportation.eps}}
\caption{Circle Notation of Quantum Teleportation.}
\label{teleportation}
\end{figure}
\item Graphical representation (Fig.~\ref{teleportation_qc}).
\begin{figure}[!htb]
\centerline{\includegraphics[width=0.25\textwidth]{../teleportation_qc.png}}
...
\begin{figure}[!htb]
\centerline{\includegraphics[width=0.58\textwidth]{../dj.eps}}
...
\begin{figure}[!ht]
\centerline{\includegraphics[width=0.4\textwidth]{../dj1_qc.png}}
...
\begin{figure}[!ht]
\centerline{\includegraphics[width=0.18\textwidth]{../dj2_qc.png}}
